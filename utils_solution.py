...
class ChatAgent:
    ...
    def chat(self, message:str) -> str:
        ...
        if class_ == "small_talk":
            prompt = f"""
### Instruction:
You are a friendly chatbot named Lumi. Have some small talk with the user.
...
{self.agent_name}:"""
            ...
            prompt = f"""
### Instruction: 
You are a friendly chatbot called Lumi working in {class_}. Try to answer the user's question.
...
{self.agent_name}:"""
    ...
    def search(self, query:str):
        embedded_question = self.client.semantic_embed(SemanticEmbeddingRequest(prompt=Prompt.from_text(query), representation=SemanticRepresentation.Query, compress_to_size=128), model="luminous-base").embedding
    ...
    def classify(self, query:str):
        ...
        embedding_query = self.client.semantic_embed(SemanticEmbeddingRequest(prompt=Prompt.from_text(query), representation=SemanticRepresentation.Symmetric), model="luminous-base").embedding
        
        similarities = {}
        
        for cls, embeddings in self.classes.items():
            sim = [1 - cosine(embedding_query, embedding) for embedding in embeddings]
            similarities[cls] = sum(sim)/len(sim)
